"""
Tests for bathymetry pipeline methods on Topo.
...
def test_source_bathy_construction(synthetic_gebco):
...
def test_source_bathy_construction_custom_names(synthetic_gebco):
    src = SourceBathy(synthetic_gebco, lon_name="x", lat_name="y", elevation_name="z")
    assert src.lon_name == "x"
    assert src.lat_name == "y"
    assert src.elevation_name == "z"


def test_source_bathy_slice_to_domain_loads_da(small_topo, synthetic_gebco):
    src = SourceBathy(synthetic_gebco).slice_to_domain(small_topo)
    assert src._da is not None


def test_source_bathy_lon_lat_arrays(src_bathy):
    assert src_bathy.lon.ndim == 1
    assert src_bathy.lat.ndim == 1


def test_source_bathy_depth_positive_down(src_bathy):
    """depth property must be positive-down (ocean depth > 0)."""
    ocean_vals = src_bathy.depth[src_bathy.depth > 0]
    assert ocean_vals.size > 0


def test_source_bathy_da_is_elevation(src_bathy):
...
def test_source_bathy_repr(src_bathy):
    r = repr(src_bathy)
    assert "SourceBathy" in r


def test_source_bathy_topo_stats_none_before_compute(src_bathy):
...
def test_generate_mask_ocean_frac_shape_matches_grid(small_topo, src_bathy):
    """Mask must have the same spatial shape as the model grid."""
    mask = small_topo.generate_mask_ocean_frac(src_bathy, nx_sub=3, ny_sub=3)
    assert mask.shape == (small_topo._grid.ny, small_topo._grid.nx)


def test_generate_mask_ocean_frac_land_strip_is_masked(small_topo, src_bathy):
...
def test_generate_mask_ocean_frac_stats_stored_on_src(small_topo, src_bathy):
...
def test_generate_mask_ocean_frac_src_stored_on_topo(small_topo, src_bathy):
...
def test_generate_mask_cartopy_returns_binary_mask(small_topo):
    """Mask values must be 0 or 1."""
    mask = small_topo.generate_mask_cartopy(resolution="50m")
    assert set(np.unique(mask.values)).issubset({0, 1})


def test_generate_mask_cartopy_shape_matches_grid(small_topo):
    """Mask must have the same spatial shape as the model grid."""
    mask = small_topo.generate_mask_cartopy(resolution="50m")
    assert mask.shape == (small_topo._grid.ny, small_topo._grid.nx)


def test_generate_mask_cartopy_open_ocean_is_unmasked(small_topo):
...
def test_diagnose_resolution_returns_bool(small_topo, src_bathy):
    result = small_topo.diagnose_resolution(src_bathy)
    assert isinstance(result, bool)


def test_diagnose_resolution_works_with_unloaded_src(small_topo, synthetic_gebco):
...
def test_write_topo_h2_in_output_when_stats_computed(small_topo, src_bathy, tmp_path):
...
def test_write_topo_h2_non_negative(small_topo, src_bathy, tmp_path):
...
def test_write_topo_enforce_topo_drag_raises_without_stats(small_topo, tmp_path):
...
def test_tidy_dataset_external_mask_overrides_depth_sign(small_topo):
...
def test_tidy_dataset_no_mask_derives_from_depth_sign(small_topo):
...
def test_set_from_dataset_dispatches_to_high_res_when_recommended(
    small_topo, synthetic_gebco, monkeypatch
):
...
def test_set_from_dataset_dispatches_to_direct_xesmf_when_not_recommended(
    small_topo, synthetic_gebco, monkeypatch
):
...
def test_set_from_dataset_default_mask_method_ocean_frac_for_high_res(
    small_topo, synthetic_gebco, monkeypatch
):
...
def test_set_from_dataset_no_default_mask_for_direct_xesmf(
    small_topo, synthetic_gebco, monkeypatch
):
...
def test_set_from_dataset_explicit_mask_method_forwarded_to_direct(
    small_topo, synthetic_gebco, monkeypatch
):
    ...
    small_topo.set_from_dataset(bathymetry_path=synthetic_gebco, mask_method="cartopy")
    assert captured.get("mask_method") == "cartopy"


def test_set_from_dataset_src_passed_to_pipeline(
    small_topo, synthetic_gebco, monkeypatch
):
...
def test_direct_xesmf_regrid_bad_mask_method_raises(small_topo, src_bathy):
...
def test_direct_xesmf_regrid_precomputed_mask_bypasses_generation(
    small_topo, src_bathy, monkeypatch
):
...
def test_high_res_regrid_bad_mask_method_raises(small_topo, src_bathy):
    """Unknown mask_method must raise ValueError."""
    with pytest.raises(ValueError, match="mask_method"):
        small_topo.high_res_regrid(src_bathy, mask_method="invalid")


def test_high_res_regrid_ocean_frac_mask_produces_valid_depth(
    small_topo, src_bathy, tmp_path
):
...
def test_high_res_regrid_cartopy_mask_produces_valid_depth(
    small_topo, src_bathy, tmp_path
):
...
def test_high_res_regrid_precomputed_mask_accepted(small_topo, src_bathy, tmp_path):
...
def test_high_res_regrid_weights_file_written(small_topo, src_bathy, tmp_path):
...
def test_mpi_direct_xesmf_regrid_exists(small_topo):
...
def test_mpi_set_from_dataset_removed(small_topo):
