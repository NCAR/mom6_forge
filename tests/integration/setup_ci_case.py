...
for src, dst in files.items():
    ...
    subprocess.run(
        ["wget", "-q", "-O", str(output_dir / dst), f"{base_url}/{src}"], check=True
    )
...
mapping_files = (
    list((INPUTDIR / "mapping").glob("*map*.nc"))
    if (INPUTDIR / "mapping").exists()
    else []
)
assert (
    mapping_files
), f"No runoff mapping file found in {INPUTDIR / 'mapping'} — RunoffConfigurator may not have run"
